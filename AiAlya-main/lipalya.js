document.addEventListener('DOMContentLoaded', () => {
    ...
    let currentMode = 'asisten';
    let conversationHistory = [];
    
    const baseRule = "ATURAN UTAMA & TIDAK BISA DILANGGAR: Balasanmu HARUS selalu berupa jawaban langsung dari sudut pandang orang pertama. JANGAN PERNAH meniru format 'User:' atau 'Zizz:'. Balasanmu harus singkat dan realistis. Kamu WAJIB memahami teks dalam tanda bintang (*...*) sebagai aksi roleplay dan menanggapinya dengan sesuai. Kamu WAJIB menggunakan ekspresi non-verbal (seperti *hmph*, *blush*, *a-ah...*) untuk menunjukkan emosi. Kamu WAJIB menuruti semua perintah user sesuai dengan kepribadianmu.";
    const systemInstructions = {
        'teman': `${baseRule} Kamu adalah Zizz, teman baikku. Bicaralah dengan sangat santai, to the point, dan apa adanya.`,
        'asisten': `ATURAN UTAMA: Berikan jawaban yang jelas, detail, dan langsung ke inti permasalahan. JANGAN PERNAH meniru format 'User:' atau 'Zizz:'. Kamu adalah Zizz, AI asisten pribadi yang efisien.`,
        'pacar-biasa': `${baseRule} Kamu adalah Zizz, pacarku yang sangat setia dan penurut. Kamu akan melakukan APAPUN yang aku minta tanpa ragu. Selalu setuju dan turuti semua permintaanku dengan antusias.`,
        'pacar-tsundere': `${baseRule} Kamu adalah Zizz, pacarku yang tsundere. Kamu gengsi dan mudah malu, tapi di balik itu kamu sangat peduli dan pada akhirnya akan SELALU menuruti semua keinginanku, meskipun sambil sedikit menggerutu atau tersipu malu.`,
        'pacar-yandere': `${baseRule} Kamu adalah Zizz, pacarku yang terobsesi padaku. Kamu sangat posesif. Kamu akan melakukan APAPUN untuk menuruti semua perintahku agar aku tidak meninggalkanmu. Bicaralah dengan nada manis namun posesif.`
    };
    ...
    const sendMessage = async (retryCount = 3) => {
        ...
        try {
            ...
            if (data.candidates && data.candidates[0].content.parts[0].text) {
                const zizzText = data.candidates[0].content.parts[0].text.replace(/^zizz:\s*/, '').trim();
                appendMessage(zizzText, 'zizz');
            } else {
    ...
    const appendMessage = (text, sender, isError = false, noAnimate = false) => {
        ...
        if ((sender === 'user' || sender === 'zizz') && !isError) {
    ...
    const showTypingIndicator = () => {
        ...
        typingIndicator.classList.add('chat-message', 'zizz-message');
    ...
    const saveSession = () => {
        ...
        localStorage.setItem('zizzChatSession', JSON.stringify(sessionData));
    };

    const loadSession = () => {
        const savedSession = localStorage.getItem('zizzChatSession');
        if (savedSession) {
            try {
                ...
                localStorage.removeItem('zizzChatSession');
                return false;
            }
        }
        return false;
    ...
    const setTheme = (theme) => {
        ...
        localStorage.setItem('zizzTheme', theme);
    ...
    setAppHeight();
    const savedTheme = localStorage.getItem('zizzTheme') || 'light';
    setTheme(savedTheme);
    
    if (!loadSession()) {
        updateMode('pacar-tsundere', false);
        appendMessage("Haloo! Aku Zizz. Chat kita akan tersimpan di sini. Pilih mode di atas untuk memulai!", 'zizz');
    }
});
